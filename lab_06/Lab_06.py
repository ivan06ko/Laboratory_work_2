# Визначаємо відповідність символів на камінь, папір, ножиці
SHAPE_SCORES = {'X': 1, 'Y': 2, 'Z': 3}  # Очки за вибір (X - Камінь, Y - Папір, Z - Ножиці)
OUTCOME_SCORES = {('A', 'X'): 3, ('A', 'Y'): 6, ('A', 'Z'): 0,
                  ('B', 'X'): 0, ('B', 'Y'): 3, ('B', 'Z'): 6,
                  ('C', 'X'): 6, ('C', 'Y'): 0, ('C', 'Z'): 3}  # Очки за результат
...
def calculate_total_score(file_path):
    ...
    try:
        with open(file_path, 'r') as file:
            for line in file:
                opponent, you = line.strip().split()  # Отримуємо вибори гравців
...
#накінецто хоч щось доробив по вебу... чому я ще раніше цим не займався... сам собі палки в колеса вставляю...